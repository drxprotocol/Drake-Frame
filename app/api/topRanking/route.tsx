...
import {GenerateFrameImg, GenerateFailedFrameImg, fetchTopFiveRankingData} from './og';
...
export async function GET(request: NextRequest) {
    try {
        let rankingData = await fetchTopFiveRankingData();

        let generateOption = await buildGenerateOption();
        return new ImageResponse(
            (
                <GenerateFrameImg rankingData={rankingData} />
            ),
            generateOption,
        );
    } catch (e) {
